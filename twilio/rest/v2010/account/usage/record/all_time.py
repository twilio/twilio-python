...
class AllTime(InstanceResource):
    """
    ...
    def __init__(self, parent, *args, **kwargs):
        self.parent = parent
        super(AllTime, self).__init__(parent, None)

    def load(self, *args, **kwargs):
        super(AllTime, self).load(*args, **kwargs)
...
class AllTimes(ListResource):
    name = "Usage/Records/AllTime"
    mount_name = "all_time"
    key = "usage_records"
    instance = AllTime

    def __init__(self, *args, **kwargs):
        super(AllTimes, self).__init__(*args, **kwargs)

    def list(self, **kwargs):
        """
        Retrieve a collection of :class:`AllTime`
        
        :raises TwilioRestException: when the request fails on execute
        
        :rtype: :class:`ListQuery`
        :returns: A ListQuery when executed returns a list of :class:`AllTime`
        """
        return self.get_instances(kwargs)

    def iter(self, **kwargs):
        """
        Return all instances of :class:`AllTime` using an iterator
        
        :raises TwilioRestException: when the request fails on execute
        
        :returns: An iterator to fetch all :class:`AllTime`
        """
        return super(AllTimes, self).iter(**kwargs)

    def load_instance(self, data):
        """ Override because AllTime does not have a sid """
