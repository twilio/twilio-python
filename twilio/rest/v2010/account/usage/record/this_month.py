...
class ThisMonth(InstanceResource):
    """
    ...
    def __init__(self, parent, *args, **kwargs):
        self.parent = parent
        super(ThisMonth, self).__init__(parent, None)

    def load(self, *args, **kwargs):
        super(ThisMonth, self).load(*args, **kwargs)
...
class ThisMonths(ListResource):
    name = "Usage/Records/ThisMonth"
    mount_name = "this_month"
    key = "usage_records"
    instance = ThisMonth

    def __init__(self, *args, **kwargs):
        super(ThisMonths, self).__init__(*args, **kwargs)

    def list(self, **kwargs):
        """
        Retrieve a collection of :class:`ThisMonth`
        
        :raises TwilioRestException: when the request fails on execute
        
        :rtype: :class:`ListQuery`
        :returns: A ListQuery when executed returns a list of :class:`ThisMonth`
        """
        return self.get_instances(kwargs)

    def iter(self, **kwargs):
        """
        Return all instances of :class:`ThisMonth` using an iterator
        
        :raises TwilioRestException: when the request fails on execute
        
        :returns: An iterator to fetch all :class:`ThisMonth`
        """
        return super(ThisMonths, self).iter(**kwargs)

    def load_instance(self, data):
        """ Override because ThisMonth does not have a sid """
