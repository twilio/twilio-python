...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def process_request(self):
        ...
        self.url = self.headers.get("x-forwarded-proto") + "://" + self.headers.get("host") + self.path
        self.send_response(200)
        self.end_headers()    
        RequestHandler.is_request_valid = RequestHandler.validator.validate(uri=self.url, params=None, signature=self.signature_header)
...
class WebhookTest(unittest.TestCase):
    ...
    def create_studio_flow(self, url, method):
        flow = self.client.studio.v2.flows.create(friendly_name="Python Cluster Test Flow", status="published", definition={
    ...
    def validate(self, method):
        ...
        self.client.studio.v2.flows(self.flow_sid).executions.create(to="to", from_="from")
