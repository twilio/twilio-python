...
class ClusterTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client = Client(username=self.api_key, password=self.api_secret, account_sid=self.account_sid)
        self.voice_twiml = VoiceResponse()

    def test_send_text_message(self):
        msg = self.client.messages.create(to=self.to_number, from_=self.from_number, body="hello world")
    ...
    def test_allow_special_characters_for_friendly_and_identity_name(self):
        ...
        conversation = self.client.conversations.v1.conversations.create(friendly_name=friendly_name)
        participant = self.client.conversations.v1.conversations(conversation.sid).participants.create(identity=identity_name)
        ...
        remove_conversation = self.client.conversations.v1.conversations(conversation.sid).delete()
        self.assertIsNotNone(remove_conversation)

    def test_list_available_numbers(self):
        toll_free_numbers = self.client.available_phone_numbers("US").toll_free.list(limit=2)
        self.assertIsNotNone(toll_free_numbers)
        self.assertEqual(len(toll_free_numbers), 2)

    def test_calling_twiml_string(self):
        call = self.client.calls.create(to=self.to_number, from_=self.from_number, twiml=str(self.voice_twiml))
        self.assertIsNotNone(call.sid)

    def test_calling_twiml_object(self):
        call = self.client.calls.create(to=self.to_number, from_=self.from_number, twiml=self.voice_twiml)
        self.assertIsNotNone(call.sid)
