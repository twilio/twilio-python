...
class WorkflowConfigTest(unittest.TestCase):
        ...
        def test_from_json(self):
            ...
            data = {'task_routing':
                         {'default_filter':
                              {'queue': 'WQ05f810d2d130344fd56e3c91ece2e594'}, 'filters': [
                             {'expression': 'type == "sales"', 'friendly_name': 'Sales', 'targets': [
                                 {'queue': 'WQec62de0e1148b8477f2e24579779c8b1',
                                  'expression': 'task.language IN worker.languages'}]},
                             {'expression': 'type == "marketing"', 'friendly_name': 'Marketing', 'targets':
                                 [{'queue': 'WQ2acd4c1a41ffadce5d1bac9e1ce2fa9f',
                                   'expression': 'task.language IN worker.languages'}]
                              },
                             {'expression': 'type == "support"', 'friendly_name': 'Support', 'targets':
